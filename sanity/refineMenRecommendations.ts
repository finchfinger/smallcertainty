...
if(!projectId||!token) throw new Error("Set NEXT_PUBLIC_SANITY_PROJECT_ID and SANITY_API_WRITE_TOKEN before refining Men’s Clothing.");

const client=createClient({projectId,dataset,token,apiVersion,useCdn:false});

type ExistingItem={_id:string; label:string; slug?:{current?:string}};

async function refineMenRecommendations(){
  const section=await client.fetch<{_id:string}|null>(`*[_type == "catalogSection" && slug.current in ["mens-clothing","men-s-clothing"]][0]{_id}`);
  ...
  menRecommendations.forEach((row,index)=>{
    const itemSlug=slugify(row.label);
    const itemId=itemIdByLabel.get(row.label)||`catalogItem-mens-clothing-${itemSlug}`;
  ...
  await tx.commit();
  console.log(`Refined Men’s Clothing with ${menRecommendations.length} rows and ${menRecommendations.length*3} recommendations.`);
}

refineMenRecommendations().catch(error=>{console.error(error);process.exit(1);});
