...
import { slugify,womenRecommendations } from "./seedData";
...
if(!projectId||!token) throw new Error("Set NEXT_PUBLIC_SANITY_PROJECT_ID and SANITY_API_WRITE_TOKEN before refining Women’s Clothing.");

const client=createClient({projectId,dataset,token,apiVersion,useCdn:false});

type ExistingItem={_id:string; label:string; slug?:{current?:string}};

async function refineWomenRecommendations(){
  const section=await client.fetch<{_id:string}|null>(`*[_type == "catalogSection" && slug.current in ["womens-clothing","women-s-clothing"]][0]{_id}`);
  ...
  womenRecommendations.forEach((row,index)=>{
    const itemSlug=slugify(row.label);
    const itemId=itemIdByLabel.get(row.label)||`catalogItem-womens-clothing-${itemSlug}`;
  ...
  await tx.commit();
  console.log(`Refined Women’s Clothing with ${womenRecommendations.length} rows and ${womenRecommendations.length*3} recommendations.`);
}

refineWomenRecommendations().catch(error=>{console.error(error);process.exit(1);});
