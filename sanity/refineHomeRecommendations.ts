...
import { homeRecommendations,slugify } from "./seedData";
...
if(!projectId||!token) throw new Error("Set NEXT_PUBLIC_SANITY_PROJECT_ID and SANITY_API_WRITE_TOKEN before refining Home.");

const client=createClient({projectId,dataset,token,apiVersion,useCdn:false});

type ExistingItem={_id:string; label:string; slug?:{current?:string}};

async function refineHomeRecommendations(){
  const homeSection=await client.fetch<{_id:string}|null>(`*[_type == "catalogSection" && slug.current == "home"][0]{_id}`);
  if(!homeSection) throw new Error("Could not find Home section.");

  const existingItems=await client.fetch<ExistingItem[]>(`*[_type == "catalogItem" && section->_id == $sectionId]{_id,label,slug}`,{sectionId:homeSection._id});
  const itemIdByLabel=new Map(existingItems.map(item=>[item.label,item._id]));
  const today=new Date().toISOString().slice(0,10);
  let tx=client.transaction();

  homeRecommendations.forEach((row,index)=>{
    const itemSlug=slugify(row.label);
    const itemId=itemIdByLabel.get(row.label)||`catalogItem-home-${itemSlug}`;
    ...
    tx=tx.createIfNotExists({
      ...
      section:{_type:"reference",_ref:homeSection._id},
      sortOrder:index+1,
      updated:false,
      published:true,
      intro:topPick.note,
  ...
  await tx.commit();
  console.log(`Refined Home with ${homeRecommendations.length} rows and ${homeRecommendations.length*3} recommendations.`);
}

refineHomeRecommendations().catch(error=>{console.error(error);process.exit(1);});
